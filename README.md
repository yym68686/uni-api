...
Supported timeout keys are `connect`, `write`, `pool`, `first_byte`, `idle`, and `total`, in seconds. The Rust runtime and channel-settings preview use one resolved policy. `model_timeout` supplies the first-byte fallback (provider, then global, then 100 seconds); it never supplies a hidden total deadline. Native `/v1/messages` applies this first-byte budget to response headers. Chat and Responses also preserve their protocol-specific precommit output checks. `idle` limits the wait between upstream body chunks; `total` limits the complete attempt from dispatch through the final body byte. These rules apply to native and translated streams. No implicit 200-second total or default idle limit is installed.

An explicit `0` disables that particular limit and overrides an inherited positive value. In particular, absent `total` or `total: 0` means no total deadline, and `idle: 0` means no idle deadline. The channel-settings API accepts and preserves zero values. Receiving headers, heartbeat chunks, or entering a protocol adapter never restarts an enabled total deadline. Once output is committed, a timeout ends the stream as a failure without replaying the request.
...
Resolution order is: global `timeout_policy.default` → most specific global `timeout_policy.rules` match → provider `timeout_policy.default` → most specific provider `timeout_policy.rules` match. Later layers override fields they explicitly set, including zero; omitted fields retain earlier values. Only the first-byte fallback uses provider/global `model_timeout`.
