...
use crate::provider_stream::{PrecommitFailure, Translation};
...
pub async fn preflight(
    ...
    loop {
        while let Some((end, separator)) = crate::provider_stream::next_event_boundary(&buffer) {
...
mod tests {
    ...
    async fn fast_failure_keeps_http_status_but_heartbeat_failure_is_explicit_sse() {
        ...
        let response = with_keepalive(
            async {
                crate::proxy::json_error(axum::http::StatusCode::GATEWAY_TIMEOUT, "timed out")
            },
            rx,
        )
        .await;
