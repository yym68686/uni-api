...
//! storage; a new GatewayRuntime starts with no overrides and a new revision.

use crate::config::snapshot::Provider;
use crate::config::snapshot::Snapshot;
use crate::routing::catalog::entries;
...
pub(crate) struct Controls {
    ...
    pub(crate) settings: BTreeMap<String, crate::control::settings::ProviderSettings>,
    ...
    pub(crate) fn settings_definitions(&self, base: &Snapshot) -> BTreeMap<String, Value> {
        ...
        for name in self.settings.keys() {
            if let Some(p) = self.temporary.get(name) {
                definitions
                    .entry(name.clone())
                    .or_insert_with(|| crate::control::settings::document(base, p));
            }
        }
        definitions
    }
    fn effective_temporary(&self, p: &Arc<Provider>, base: &Snapshot) -> Arc<Provider> {
        if let Some(setting) = self.settings.get(p.name.as_ref()) {
            let raw = self
                ...
                .get(p.name.as_ref())
                .cloned()
                .unwrap_or_else(|| crate::control::settings::document(base, p));
            crate::control::settings::compile(
                &crate::control::settings::merge(&raw, &setting.set, &setting.remove),
                p,
            )
    ...
    pub fn overlay(&self, base: Arc<Snapshot>) -> Arc<Snapshot> {
        ...
        for provider in self.temporary.values() {
            ...
            let Some(token) = base
                .api_key_order
                .iter()
                .find(|token| crate::routing::catalog::key_id(token) == owner)
            else {
                continue;
            };
        ...
        for provider in &mut providers {
            if let Some(settings) = self.settings.get(provider.name.as_ref()) {
                let raw = self
                    ...
                    .cloned()
                    .unwrap_or_else(|| crate::control::settings::document(&base, provider));
                let effective =
                    crate::control::settings::merge(&raw, &settings.set, &settings.remove);
                let next =
                    crate::control::settings::compile(&effective, provider).unwrap_or_else(|_| {
                        settings
                            .compiled
                            .clone()
    ...
    pub(crate) async fn mutate_controls(
        ...
        if input.action == "set" {
            let caller = snapshot
                .api_keys
                .values()
                .find(|k| crate::routing::catalog::can_inspect_all(&snapshot, k))
...
pub(crate) fn temporary_allowed(
    provider: &Provider,
    key: &crate::config::snapshot::ApiKey,
) -> bool {
    provider
        .preferences
        .get("__temporary_key_id")
        .and_then(Value::as_str)
        .is_none_or(|owner| {
            owner == crate::routing::catalog::key_id(&key.token)
                || key
                    .preferences
...
impl GatewayRuntime {
    pub(crate) async fn import_temporary_channel(
        ...
        let selected = snapshot
            .api_keys
            .values()
            .find(|k| crate::routing::catalog::key_id(&k.token) == input.api_key_id)
...
pub(crate) struct RetainedSnapshot {
    ...
    pub channel_settings: BTreeMap<String, crate::control::settings::ProviderSettings>,
}
    ...
    pub(crate) async fn restore_controls(
        ...
        for p in input.snapshot.temporary_channels {
            if !(p.provider.starts_with("sub2api-") || p.definition.is_some())
                || !crate::control::settings::valid_created_name(&p.provider)
            ...
            if !base
                .api_keys
                .values()
                .any(|k| crate::routing::catalog::key_id(&k.token) == p.api_key_id)
            {
            ...
            let provider = if let Some(raw) = p.definition {
                let built =
                    crate::control::settings::compile(&raw, &provider).map_err(|e| bad(&e))?;
                candidate
                    .temporary_documents
