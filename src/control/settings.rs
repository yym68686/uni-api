...
pub(crate) fn compile(raw: &Value, previous: &Arc<Provider>) -> Result<Arc<Provider>, String> {
    ...
    let compiled =
        crate::config::compiler::compile_provider(&materialized).ok_or(field_error("/model"))?;
    let parsed = serde_json::from_value(compiled).map_err(|_| field_error("/"))?;
    let mut next =
        (*crate::config::snapshot::runtime_provider(parsed, previous.cursor.clone())).clone();
    ...
    if let Some(v) = next.preferences.get("api_key_rate_limit") {
        ...
        if values
            .iter()
            .any(|v| crate::runtime::scheduling::parse_rate_limits(Some(v), None).is_none())
        {
...
fn affected(snapshot: &Snapshot, provider: &str) -> Vec<Value> {
    let Some(inspector) = snapshot
        .api_keys
        .values()
        .find(|key| crate::routing::catalog::can_inspect_all(snapshot, key))
    else {
        return Vec::new();
    };
    snapshot
        ...
        .filter_map(|token| {
            ...
            let id = crate::routing::catalog::key_id(token);
            let entries = crate::routing::catalog::entries(snapshot, inspector, Some(&id)).ok()?;
...
impl GatewayRuntime {
    ...
    pub(crate) async fn settings_discover(&self, input: Mutation) -> Result<Value, Failure> {
        ...
        let models = crate::config::discovery::discover_provider_models(&client, &raw)
            .await
    ...
    pub(crate) async fn settings_change(
        ...
        for original_change in &input.changes {
            if !original_change.create_to_key.is_empty() {
                ...
                let compiled = crate::config::compiler::compile_provider(&raw)
                ...
                let mut prototype =
                    (*crate::config::snapshot::runtime_provider(parsed, Arc::default())).clone();
            ...
            let prepared = crate::providers::request::preview_channel_request(
                &compiled,
                request_model,
                upstream,
                endpoint,
            ...
            if let Some(o) = body.as_object_mut() {
                ...
                crate::providers::overrides::apply_overrides(o, &compiled, request_model);
            }
            ...
            previews.push(json!({"provider":c.provider,"created":!original_change.copy_to_key.is_empty() || !original_change.create_to_key.is_empty(),"before":redact_at(&current,""),"after":redact_at(&merged,""),"sample":{"model":request_model,"upstream_model":upstream,"endpoint":endpoint,"timeouts":timeout,"excluded_endpoint":compiled.excluded_endpoints.iter().any(|e|e==endpoint),"excluded_request_type":!crate::routing::filters::provider_accepts_request_type(&compiled,input.sample.get("request_type").and_then(Value::as_str)),"engine":compiled.engine.as_ref(),"base_url":compiled.base_url.as_ref(),"wire_request":wire,"excluded_rule":!crate::routing::filters::provider_accepts_request_rules(&compiled,endpoint,request_model,input.sample.get("reasoning_effort").and_then(Value::as_str),input.sample.get("request_type").and_then(Value::as_str)),"body":redact_at(&json!({"preferences":{"post_body_parameter_overrides":body}}),"")["preferences"]["post_body_parameter_overrides"]}}));
        }
...
mod tests {
    ...
    use crate::config::snapshot::RawProvider;
    ...
    async fn all_supported_engines_can_be_created_restored_and_removed() {
        ...
        for engine in schema()["engines"].as_array().unwrap() {
            ...
            let owner = crate::routing::catalog::key_id("caller-a");
            ...
            assert!(crate::control::channels::temporary_allowed(
                provider,
    ...
    async fn generic_creation_rejects_invalid_definitions_atomically() {
        ...
        for (name, settings) in [
            ...
            let input = serde_json::from_value(json!({"revision":revision,"operation_id":"invalid","changes":[{"provider":name,"create_to_key":crate::routing::catalog::key_id("caller-a"),"set":settings}]})).unwrap();
        ...
        for (field, value) in [
            ...
            let input=serde_json::from_value(json!({"revision":revision,"operation_id":"invalid","changes":[{"provider":"valid-name","create_to_key":crate::routing::catalog::key_id("caller-a"),"set":settings}]})).unwrap();
            assert!(store.settings_change(input, true).await.is_err());
            assert_eq!(
    ...
    async fn fixture() -> GatewayRuntime {
        ...
        let item: RawProvider =
            serde_json::from_value(crate::config::compiler::compile_provider(&raw).unwrap())
                .unwrap();
        let p = crate::config::snapshot::runtime_provider(item, Arc::new(AtomicUsize::new(0)));
    ...
    async fn copy_is_only_available_to_destination_key_and_restores_full_definition() {
        ...
        input.changes[0].copy_to_key = crate::routing::catalog::key_id("caller-a");
        ...
        restored
            .restore_controls(
                &headers,
                crate::control::channels::RestoreMutation {
                    revision: view["revision"].as_str().unwrap().into(),
                    snapshot: crate::control::channels::RetainedSnapshot {
                        version: 2,
                        rules: vec![],
                        channel_settings: settings,
                        temporary_channels: vec![crate::control::channels::RetainedChannel {
                            provider: name.into(),
                            api_key_id: crate::routing::catalog::key_id("caller-a"),
    ...
    fn checks_invalid_values_and_preserves_false_zero() {
        ...
        let p = crate::config::snapshot::runtime_provider(
            serde_json::from_value(crate::config::compiler::compile_provider(&raw).unwrap())
                .unwrap(),
