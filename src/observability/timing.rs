...
use crate::observability::metrics::ChannelMetrics;
use crate::observability::metrics::MetricKey;
use serde_json::json;
...
impl RequestArrival {
    ...
    pub(crate) fn attempt(
        ...
        let key_id = crate::routing::catalog::key_id(api_key);
        let billing = Arc::new(crate::observability::billing::BillingAttempt::new(
            key.clone(),
            request_id.clone(),
            attempt_id.clone(),
            key_id.clone(),
...
pub(crate) struct AttemptDispatch {
    ...
    pub(crate) billing: Arc<crate::observability::billing::BillingAttempt>,
}
    ...
    pub(crate) fn record(&self, metrics: &ChannelMetrics) -> f64 {
        ...
        *self.recorded.get_or_init(|| {
            ...
            if let Some(writer) = crate::storage::s3::global() {
                writer.enqueue(crate::storage::s3::dispatch_event(
                    &self.key,
                    &self.request_id,
                    &self.attempt_id,
                    &self.key_id,
