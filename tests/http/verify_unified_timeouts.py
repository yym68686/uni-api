...
def verify(binary):
    ...
    scenarios = {
        ...
        'total-includes-header-wait': {'policy': {'total': .25}, 'headers': .15, 'gap': .06, 'chunks': 8, 'cut': True},
    ...
    for variant, endpoint, upstream, engine in variants:
        for name, timing in scenarios.items():
            ...
            for role in ['first', 'fallback']:
                provider = role + '-' + model
                preferences = {'model_timeout': .2, 'cooldown_period': 0, 'api_key_cooldown_period': 0}
                if case.get('policy'):
    ...
    server.cases[model] = {'endpoint': '/v1/responses', 'stream': True, 'body': .4, 'fallback': True}
    for role in ['first', 'fallback']:
        provider = role + '-' + model
        providers.append({'provider': provider, 'engine': 'codex', 'api': 'fixture-upstream', 'model': [model],
                          ...
                          'preferences': {'timeout_policy': {'default': {'first_byte': 1, 'idle': .15, 'total': 0}}}})
        routes.append(provider + '/*')
            ...
            try:
                ...
                for model, case in server.cases.items():
                    ...
                    if case.get('cut'):
                        ...
                        assert hits == ['first'] and elapsed < .38, context
                    else:
